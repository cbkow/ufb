...
/// Unix-socket subscriber that listens to the agent's broadcast
/// channel for `BadgeUpdate` messages and forwards them to the
/// FinderSync controller. Mirrors the tray's `AgentConnection` but
/// read-mostly — only sends commands for drain/stats actions
/// triggered from the Finder context menu.
///
/// Push model: agent broadcasts BadgeUpdate on every hydration /
/// eviction. We don't maintain a local cache; the controller calls
/// `setBadgeIdentifier(_:for:)` on each update and lets FinderSync
/// own the current visible state.
///
/// Cold-start caveat: if FinderSync launches after the agent has
/// been running, it won't see badges for already-hydrated files
/// until those files transition state. Acceptable for v1 — badges
/// are a visual cue, not correctness. A full-state request message
/// is a future add if users report visible gaps.
///
/// Per macOSplans/05 — ported from
/// `ufb-tauri/mediamount-tray/FinderSync/BadgeClient.swift` with
/// rename pass.
class BadgeClient {
    /// Called when the agent reports a hydration-state change. The
    /// URL is the absolute path under `~/ufb/mounts/`. A nil
    /// identifier means "drop any existing badge."
    var onBadgeChange: ((URL, String?) -> Void)?

    /// Controller provides the mount-root prefix so BadgeClient can
    /// map `(domain, relpath)` pairs back to absolute `file://` URLs.
    ...
    private let ioQueue = DispatchQueue(label: "dev.ufb.tray.findersync.badge")

    init() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        // Short filename matches unix_server::socket_path — keeps the
        // full path under macOS's 104-byte sun_path limit.
        socketPath =
            "\(home)/Library/Group Containers/5Z4S9VHV56.group.com.unionfiles.ufb/a.sock"
    }
    ...
    private func tryConnect() -> Bool {
        ...
        // Bootstrap: ask the agent to resend current mount states.
        // The agent's broadcast channel will then forward new
        // BadgeUpdates as they occur.
    ...
    /// Send a ClearSyncCache for the given share (mount id OR share
    /// name — the agent accepts both for backward compat). Invoked
    /// from the "Drain Cache" context menu.
