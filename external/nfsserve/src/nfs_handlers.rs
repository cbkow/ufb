...
pub async fn handle_nfs(
    ...
    match prog {
        ...
        // UFB fork addition: dispatch COMMIT to the new handler.
...
pub async fn nfsproc3_write(
    ...
    // UFB fork: propagate the client's requested `stable` to
    // write_with_stable. Default impl returns FILE_SYNC matching the
    // pre-fork behavior; overriding impls can honor UNSTABLE to skip
    // the per-WRITE fsync (the client must then issue COMMIT).
    let requested_stable =
        stable_how::from_u32(args.stable).unwrap_or(stable_how::FILE_SYNC);
    match context
        .vfs
        .write_with_stable(id, args.offset, &args.data, requested_stable)
        .await
    {
        Ok((fattr, actual_stable)) => {
...
// ── UFB fork: NFSPROC3_COMMIT handler ──
//
// COMMIT3args { file: nfs_fh3, offset: offset3, count: count3 }
// COMMIT3resok { file_wcc: wcc_data, verf: writeverf3 }
//
// Tells the server "flush bytes [offset, offset+count) for `file`".
// count=0 means "the whole file from offset". Returning a verf that
// matches a prior WRITE's verf confirms the data survived; a different
// verf tells the client to re-send the writes (server restart).
...
pub async fn nfsproc3_commit(
    ...
    args.deserialize(input)?;
    debug!("nfsproc3_commit({:?}, offset={}, count={})", xid, args.offset, args.count);
