...
pub trait NFSFileSystem: Sync {
    ...
    /// Fork-only addition (UFB): write with explicit `stable_how`.
    /// Default impl forwards to `write()` and returns FILE_SYNC, so
    /// existing impls Just Work. Impls that want async-write semantics
    /// can override to skip per-WRITE fsync when `stable == UNSTABLE`
    /// and instead rely on a follow-up COMMIT call.
    ///
    /// Returns `(fattr3, actual_stable)`. The actual_stable echoed back
    /// to the NFS client tells it whether to issue a COMMIT later
    /// (UNSTABLE → yes, FILE_SYNC → no).
    async fn write_with_stable(
        &self,
        id: fileid3,
        offset: u64,
        data: &[u8],
        _stable: crate::nfs_handlers::stable_how,
    ) -> Result<(fattr3, crate::nfs_handlers::stable_how), nfsstat3> {
        let attr = self.write(id, offset, data).await?;
        Ok((attr, crate::nfs_handlers::stable_how::FILE_SYNC))
    }

    /// Fork-only addition (UFB): NFSPROC3_COMMIT handler. Default impl
    /// returns the server verf with no I/O (matches existing
    /// behaviour, since the upstream nfsserve 0.10.2 dispatcher
    /// never called COMMIT and impls had no chance to flush). Impls
    /// using write_with_stable=UNSTABLE should override to fsync the
    /// requested range so the client's COMMIT is honored.
    async fn commit(
        &self,
        _id: fileid3,
        _offset: u64,
    ...
    async fn fsinfo(
        ...
        let res = fsinfo3 {
            ...
            rtmax: 1024 * 1024,
            // Was `1024 * 124` (~124 KB) — an upstream typo. Advertising
            // a preferred read far below rtmax made clients issue 8×
            // more READ RPCs than needed for large-file streaming.
            rtpref: 1024 * 1024,
            rtmult: 1024 * 1024,
            wtmax: 1024 * 1024,
            wtpref: 1024 * 1024,
            wtmult: 1024 * 1024,
            dtpref: 1024 * 1024,
            // Was 128 GiB, which clients enforce — camera-card backups,
            // ProRes reels and disk images exceed it routinely. The
            // backing store (SMB/NAS) is the real limit.
            maxfilesize: u64::MAX,
