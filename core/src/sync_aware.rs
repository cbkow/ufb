...
//!
//! When source and destination are both within a Cloud Files / WinFsp
//! sync root, optimised operations avoid hydrating dehydrated files:
...
pub fn sync_copy(src: &Path, dest_dir: &Path) -> Result<bool, String> {
    ...
    if src.is_dir() {
        // Directories: too complex to replicate the full subtree as
        // placeholders, fall back to normal copy.
        return Ok(false);
    }
    ...
    let meta =
        std::fs::metadata(&nas_path).map_err(|e| format!("Can't read NAS file: {}", e))?;
...
/// Must match the logic in `agent::config`.
