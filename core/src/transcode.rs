use crate::events::TranscodeEventsArc;
...
// so no `CommandExt` trait import is needed (unlike `std::process::Command`).
...
impl TranscodeState {
    ...
    fn update_job(
        &mut self,
        job_id: &str,
        f: impl FnOnce(&mut TranscodeJob),
    ) -> Option<TranscodeJob> {
...
impl TranscodeManager {
    ...
    pub async fn cancel_job(&self, id: &str) -> Result<(), String> {
        ...
        let should_kill = matches!(
            job.map(|j| &j.status),
            Some(JobStatus::Processing | JobStatus::CopyingMetadata)
        );

        state.update_job(id, |job| match job.status {
    ...
    async fn process_job(&self, job_id: &str) {
        ...
        let total_frames = match self.get_frame_count(&input_path).await {
            ...
            Err(e) => {
                log::warn!(
                    "ffprobe frame count failed ({}), using 0: {}",
                    input_path,
                    e
                );
                0
        ...
        if let Err(e) = self
            .run_ffmpeg(job_id, &input_path, &output_path, total_frames)
            .await
        {
            let is_cancelled = {
                let state = self.state.lock().await;
