...
bool AudioPlayer::open(const QString &path, int audioStreamCountHint)
{
    ...
    m_decoder->start();
    // Fresh decoder at 0: the QML lightbox resumes without a seek
    // after a source change, so anchor here rather than trip the
    // discontinuity re-seek on the first update().
    resetAnchor(0.0);
    emit hasAudioChanged();
    return true;
}
...
void AudioPlayer::resetAnchor(double srcSec)
{
...
void AudioPlayer::play()
{
    ...
    // seek() / resetAnchor() (consumption simply pauses with the
    // device while stopped, so the estimate stays valid across
    // pause/play).
...
{
    // Continuous sync servo (ported from QCView), called per video
    // frame or from AudioPreview's ~30 Hz pump. Three tiers by |drift|:
    //
    ...
    // consume — the position estimate freezes while the clock runs
    // on, and correcting would just thrash seeks against EOF. Freeze
    // the servo instead; the next real seek (loop wrap, user action)
    // re-engages it. Matters for AudioPreview, which plays audio-only
    // files right to the tail every time.
    ...
    const double latencySec = m_device->bufferLatencySeconds() * ratioNow;
