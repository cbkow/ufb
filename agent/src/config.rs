...
pub struct MountConfig {
    ...
    /// `unmanaged` entries, any native path taken verbatim.
    pub nas_share_path: String,

    /// "Fake" mount (plans/17): listed under Mounts for UI parity but
    /// the agent never mounts / heartbeats / authenticates it — the
    /// path is used as-is (a local folder, an OS-managed volume, a
    /// mount owned by another tool). Essentially a bookmark that
    /// lives in the Mounts section.
    #[serde(default)]
    pub unmanaged: bool,

    /// Key used to store/retrieve credentials from the credential store.
    /// New mounts stopped writing this in slice B (credentials are
    /// OS-owned, keyed by server) — must default or their configs
    /// fail to deserialize.
    #[serde(default)]
...
// Parts of the config API are macOS/Linux-only consumers now (slice B
// made Windows letter-based): share_name_collisions, volume_path &
// friends. Windows builds see them as dead; slice G prunes for real.
...
impl MountsConfig {
    ...
    pub fn share_name_collisions(&self) -> Vec<(String, Vec<String>)> {
        ...
        for m in &self.mounts {
            // Unmanaged entries never claim a /Volumes name.
            if m.enabled && !m.unmanaged {
                by_name
                    .entry(m.share_name())
                    .or_default()
                    .push(m.id.clone());
...
#[cfg_attr(windows, allow(dead_code))] // see MountsConfig note above
impl MountConfig {
    ...
    /// macOS: NFS loopback mount point owned by nfs_server (the
    /// orchestrator never queries this — kept for symmetry with the
    /// Windows path so any future cross-platform caller has one
    /// function to call).
    pub fn sync_root_dir(&self, cache_root: &std::path::Path) -> PathBuf {
        #[cfg(target_os = "macos")]
        {
            // Slice G: returns the NFS loopback mount point owned by
            // nfs_server::mount_point_for. The pre-refactor code
            // returned a FileProvider domain path for an extension
            // that was deleted in Slice G.
            let _ = cache_root;
    ...
    /// e.g., \\192.168.1.50\test1 → test1
    ///       \\nas\Projects → Projects
    ///       \\nas\Tank\Deep\DEEP_JOBS → DEEP_JOBS
...
mod tests {
    ...
    fn test_config_round_trip() {
        let config = MountsConfig {
            ...
            mounts: vec![MountConfig {
                ...
                nas_share_path: r"\\nas\media".into(),
                credential_key: "ufb_primary-nas".into(),
    ...
    fn test_defaults_applied() {
        // Minimal config — only required fields. No credentialKey:
        // the Add dialog stopped writing it in slice B, so a config
        ...
        assert!(m.credential_key.is_empty()); // default ""
    ...
    #[test]
    fn test_share_name() {
        let m = test_mount("primary-nas", r"\\192.168.1.50\Projects");
        assert_eq!(m.share_name(), "Projects");

        let m = test_mount("deep", r"\\nas\Tank\Deep\DEEP_JOBS");
        assert_eq!(m.share_name(), "DEEP_JOBS");
    ...
    fn test_volume_path_uses_share_name() {
        ...
        assert!(vp.ends_with("Projects"), "volume_path should end with share_name, got: {}", vp);
    ...
    fn test_mount_path_macos() {
        let m = test_mount("primary-nas", r"\\192.168.1.50\Projects");
        let path = m.mount_path();
        assert!(path.ends_with("ufb/mounts/Projects"), "expected path ending with ufb/mounts/Projects, got: {}", path);
    }
    ...
    #[test]
    #[cfg(target_os = "macos")]
    fn test_sync_root_dir_macos_points_at_nfs_mount() {
        let m = test_mount("primary-nas", r"\\nas\Projects");
        let root = m.sync_root_dir(std::path::Path::new("/ignored"));
        let s = root.to_string_lossy();
        assert!(
            s.ends_with("ufb/mounts/Projects"),
            "unexpected sync_root_dir on macOS: {}", s
        );
    }
    ...
    fn test_share_name_collisions() {
        let config = MountsConfig {
            ...
            sync_cache_root: None,
            mounts: vec![
                test_mount("nas-a", r"\\server-a\Projects"),
                test_mount("nas-b", r"\\server-b\Projects"),
    ...
    fn test_no_collisions_when_disabled() {
        let mut m = test_mount("nas-b", r"\\server-b\Projects");
        m.enabled = false;
        let config = MountsConfig {
            version: 1,
            sync_cache_root: None,
